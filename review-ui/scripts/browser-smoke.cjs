...
async function main() {
  ...
  try {
    ...
    await reviewWorkspace.getByRole('checkbox').first().click()
    await reviewWorkspace.getByText('Set 1', { exact: true }).waitFor()
    await reviewWorkspace.getByRole('button', { name: 'Reset to unresolved' }).click()
    await reviewWorkspace.getByRole('button', { name: 'Mark reviewed — keep all' }).click()
    ...
    await setNavigation.getByRole('button', { name: 'Previous' }).click()
    await reviewWorkspace.getByText('Set 1', { exact: true }).waitFor()
    await reviewWorkspace.getByRole('button', { name: 'Keep only this' }).first().click()
    await reviewWorkspace.getByText('Set 2', { exact: true }).waitFor()
    await page.getByRole('button', { name: 'Undo' }).click()
    await setNavigation.getByRole('button', { name: 'Previous' }).click()
    await reviewWorkspace.getByText('Set 1', { exact: true }).waitFor()
